...
fn the_web_leaderboard_label_varies_between_runs() {
    let source = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/game/shift.rs"));
    let branch = source
...
fn version_one_save_migrates_without_inventing_a_run() {
    let player_stats = nightmare_shift::state::PlayerStats::new();
...
fn run_checkpoint_round_trip_keeps_rng_and_simulation_time() {
    let mut state = nightmare_shift::state::GameState::new(
        12.5,
        &nightmare_shift::data::GameConstants::default(),
    );
    let _ = state.rng.next_u64();
    state.guideline_time_remaining = 7.0;
    let save = super::SaveData::with_run(nightmare_shift::state::PlayerStats::new(), &state);
...
fn newer_save_versions_are_rejected_before_loading() {
    let save = super::SaveData {
        ...
        player_stats: nightmare_shift::state::PlayerStats::new(),
        run: None,
    };
