...
use nightmare_shift::data::loader::load_localization;
use nightmare_shift::screens::Screen;
use nightmare_shift::state::{GamePhase, KeyBindings};
...
fn every_phase_that_numbers_its_options_reads_the_digits() {
    ...
    for (phase, screen_source) in [
        (
            ...
            include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/src/screens/game_screens/driving.rs"
            )),
        ),
        (
...
fn gamepad_confirm_and_cancel_follow_visible_gameplay_actions() {
    ...
    assert_eq!(actions, vec![nightmare_shift::ui::UiAction::AcceptRide]);
    ...
    assert_eq!(
        actions,
        vec![nightmare_shift::ui::UiAction::TogglePauseMenu]
    );
}
...
fn gamepad_directions_choose_numbered_route_options() {
    ...
    assert_eq!(actions, vec![nightmare_shift::ui::UiAction::SelectRoute(3)]);
}
